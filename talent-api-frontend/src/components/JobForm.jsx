...
const JobForm = ({ job = {}, onSubmit }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (job.id) {
        await axios.put(`/job-listings/${job.id}`, jobData); // Update existing job listing
      } else {
        await axios.post('/job-listings', jobData); // Create new job listing
      }
      onSubmit(); // Callback to refresh the list or close the form
    } catch (error) {
